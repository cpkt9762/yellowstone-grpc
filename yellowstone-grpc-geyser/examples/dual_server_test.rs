...
async fn main() -> Result<()> {
    ...
    if args.len() != 3 {
        eprintln!("Usage: {} <tcp_address> <unix_socket_path>", args[0]);
        eprintln!("Example: {} 127.0.0.1:10000 /tmp/yellowstone-grpc.sock", args[0]);
        std::process::exit(1);
    }

    let tcp_address = args[1].parse()?;
    let unix_socket_path = PathBuf::from(&args[2]);
    
    info!("Starting dual gRPC server:");
    info!("  TCP address: {}", tcp_address);
    info!("  Unix socket: {:?}", unix_socket_path);
    ...
    // Create a config that supports both TCP and Unix socket
    let config_grpc = ConfigGrpc {
        address: Some(tcp_address),
    ...
    info!("✅ Dual gRPC server started successfully!");
    info!("  TCP clients can connect to: {}", tcp_address);
    info!("  Unix clients can connect to: {:?}", unix_socket_path);
    info!("Server will run for 180 seconds for testing...");

    // Wait for 180 seconds for testing
...
async fn generate_fake_data(
    ...
    let mut slot_counter = 200_000_000u64;
            ...
            _ = interval.tick() => {
                ...
                let account_message = Message::Account(MessageAccount {
                    account: Arc::new(MessageAccountInfo {
                        ...
                        lamports: 2_000_000 + (account_counter as u64 * 150_000),
                        owner: Pubkey::try_from(fake_owner.as_slice()).unwrap_or_default(),
                        executable: account_counter % 8 == 0, // Every 8th account is executable
                        rent_epoch: 400 + (account_counter as u64 % 15),
                        data: fake_data,
...
fn generate_fake_pubkey(counter: u32) -> Vec<u8> {
    ...
    pubkey[4] = 0x02; // Mark as dual server fake
    pubkey
}
...
fn generate_fake_owner(counter: u32) -> Vec<u8> {
    ...
    match counter % 6 {
        ...
        4 => {
            // Vote Program
            owner[0..4].copy_from_slice(&[0x76, 0x1f, 0x58, 0x59]);
            owner[4..8].copy_from_slice(&[0x4e, 0x59, 0x2d, 0x43]);
        }
        _ => {
            // Custom program
            owner[0..4].copy_from_slice(&counter.to_be_bytes());
            owner[4] = 0xDD; // Mark as dual server custom
        }
    }
...
fn generate_fake_account_data(counter: u32) -> Vec<u8> {
    let data_size = match counter % 5 {
        0 => 0,     // Empty account
        1 => 32,    // Small account (like a mint)
        2 => 82,    // Mint account size
        3 => 165,   // Token account size
        _ => 2048,  // Large account
    };
    ...
    if !data.is_empty() {
        ...
        if data_size >= 32 {
            data[4..8].copy_from_slice(&[0xCA, 0xFE, 0xBA, 0xBE]); // Magic bytes for dual server
            data[8..16].copy_from_slice(&(counter as u64 * 2000).to_le_bytes()); // Amount
        }
